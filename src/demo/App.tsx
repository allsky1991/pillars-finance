...
const USDpiIcon = ({ size = 24, className, noBg = false }: { size?: number, className?: string, noBg?: boolean }) => {
  const content = (
    <svg viewBox="0 0 100 100" className="w-full h-full p-[18%]" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect x="10" y="15" width="80" height="10" rx="2" fill={noBg ? "currentColor" : "#13151a"} />
      <rect x="10" y="75" width="80" height="10" rx="2" fill={noBg ? "currentColor" : "#13151a"} />
      <rect x="20" y="30" width="15" height="40" rx="2" fill={noBg ? "currentColor" : "#13151a"} />
      <rect x="42.5" y="30" width="15" height="40" rx="2" fill={noBg ? "currentColor" : "#13151a"} />
      <rect x="65" y="30" width="15" height="40" rx="2" fill={noBg ? "currentColor" : "#13151a"} />
    </svg>
  );
  
  if (noBg) return <div className={className} style={{ width: size, height: size }}>{content}</div>;

  return (
    <div className={cn("relative flex items-center justify-center rounded-full bg-jup-primary shadow-lg overflow-hidden shrink-0", className)} style={{ width: size, height: size }}>
      {content}
    </div>
  );
};

const SUsdPiIcon = ({ size = 24, className, noBg = false }: { size?: number, className?: string, noBg?: boolean }) => {
  const content = (
    <svg viewBox="0 0 100 100" className="w-full h-full p-[18%]" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect x="10" y="15" width="80" height="10" rx="2" fill={noBg ? "currentColor" : "#c7f284"} />
      <rect x="10" y="75" width="80" height="10" rx="2" fill={noBg ? "currentColor" : "#c7f284"} />
      <rect x="20" y="30" width="15" height="40" rx="2" fill={noBg ? "currentColor" : "#c7f284"} />
      <rect x="42.5" y="30" width="15" height="40" rx="2" fill={noBg ? "currentColor" : "#c7f284"} />
      <rect x="65" y="30" width="15" height="40" rx="2" fill={noBg ? "currentColor" : "#c7f284"} />
    </svg>
  );
  ...
  return (
    <div className={cn("relative flex items-center justify-center rounded-full bg-[#13151a] border-2 border-jup-primary shadow-lg overflow-hidden shrink-0", className)} style={{ width: size, height: size }}>
      {content}
    </div>
  );
};
...
const EARN_POOLS = [
  { id: 1, pair: 'USDpi/USDC', protocol: 'Aerodrome', apy: '12.4%', tvl: '$4.2M', voted: '1.2M vePILL' },
  { id: 2, pair: 'USDpi', protocol: 'Aave', apy: '8.2%', tvl: '$12.8M', voted: '850K vePILL' },
  { id: 3, pair: 'USDpi/USDT', protocol: 'Curve Finance', apy: '4.5%', tvl: '$25.1M', voted: '2.1M vePILL' },
];
...
const Navbar = ({ 
  ...
  const networks = [
    { id: 'base', name: 'Base', color: 'text-[#0052FF]', icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
        <circle cx="12" cy="12" r="10" />
      </svg>
    )},
    { id: 'bnb', name: 'BNB', color: 'text-[#F3BA2F]', icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
    ...
    { id: 'sui', name: 'Sui', color: 'text-[#6FBCF0]', icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
    ...
    { id: 'monad', name: 'Monad', color: 'text-[#836EF9]', icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 border-b border-jup-border sticky top-0 bg-jup-bg/80 backdrop-blur-md z-50">
      <div className="flex items-center gap-8">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActiveTab('stablecoin')}>
          <USDpiIcon size={32} />
          <span className="text-xl font-bold tracking-tight uppercase">PILLARS</span>
        </div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative">
          ...
          >
            <div className={cn("flex items-center justify-center", selectedNetwork.color)}>
              {selectedNetwork.icon}
            </div>
            <span className="text-sm font-bold hidden sm:inline">{selectedNetwork.name}</span>
            <ChevronDown size={14} className={cn("transition-transform", showNetworkDropdown && "rotate-180")} />
          </button>
                ...
                {networks.map((net) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedNetwork.id === net.id ? "text-jup-primary bg-jup-primary/5" : "text-jup-text-dim"
                    )}
                  >
                    <div className={cn("flex items-center justify-center", net.color)}>
                      {net.icon}
                    </div>
                    {net.name}
                  </button>
        ...
        {isConnected ? (
          <div className="relative">
            ...
            >
              ...
              {walletAddress}
              <ChevronDown size={14} className={cn("transition-transform", showWalletDropdown && "rotate-180")} />
            </button>
...
const TokenSelectorModal = ({ isOpen, onClose, onSelect }: { isOpen: boolean, onClose: () => void, onSelect: (asset: typeof ASSETS[0]) => void }) => {
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            <div className="p-5 border-b border-jup-border flex items-center justify-between">
              ...
              <button onClick={onClose} className="text-jup-text-dim hover:text-white transition-colors p-1">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-3 max-h-[60vh] overflow-y-auto space-y-2">
              {ASSETS.map((asset) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={cn("w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-lg", asset.color)}>
                      {asset.icon}
                    </div>
                    <div className="text-left">
                      <div className="font-bold text-base">{asset.symbol}</div>
                      <div className="text-xs text-jup-text-dim">{asset.name}</div>
                      <div className="flex items-center gap-1 mt-1">
                        <span className="text-[10px] bg-jup-secondary px-1.5 py-0.5 rounded text-jup-text-dim font-mono">{asset.address}</span>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-jup-text-dim">{asset.price}</div>
                  </div>
                </button>
              ))}
            </div>
...
const StablecoinView = () => {
  ...
  const handleHalf = () => {
    if (mode === 'Mint') {
      ...
      // For burn, assume a mock USDpi balance of 1000
      setAmount('500');
    }
  };
          ...
          <div className="jup-card p-6 bg-[#13151a]">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-3">
                        ...
                        {['0.1%', '0.2%', '0.5%', '1%'].map((s) => (
                          <button
                            ...
                            className={cn(
                              ...
                              slippage === s ? "text-jup-primary bg-jup-primary/10" : "text-jup-text-dim hover:text-white hover:bg-white/5"
                            )}
                          >
                            {s}
                          </button>
                ...
                <div className="relative">
                  ...
                  >
                    {mode} <ChevronDown size={20} className={cn("transition-transform", showModeDropdown && "rotate-180")} />
                  </button>
                        ...
                        {['Mint', 'Burn'].map((m) => (
                          <button
                            ...
                            className={cn(
                              ...
                              mode === m ? "text-jup-primary" : "text-jup-text-dim"
                            )}
                          >
                            {m}
                          </button>
            ...
            <div className="space-y-1">
              <div className="bg-jup-secondary/40 p-4 rounded-2xl border border-jup-border/50">
                <div className="flex justify-between text-sm font-bold mb-3">
                  ...
                  <span className="text-jup-text-dim">${mode === 'Mint' ? (parseFloat(amount) * assetPrice || 0).toFixed(2) : amount || 0}</span>
                </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  {mode === 'Mint' ? (
                    ...
                    >
                      <div className={cn("w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold text-white", selectedAsset.color)}>
                        {selectedAsset.icon}
                      </div>
                ...
                <div className="flex justify-between items-center mt-4 text-xs font-bold">
                  <span className="text-jup-text-dim">
                    Balance: {mode === 'Mint' ? selectedAsset.balance : '1,000.00'} {mode === 'Mint' ? selectedAsset.symbol : 'USDpi'}
                  </span>
              ...
              <div className="bg-jup-secondary/40 p-4 rounded-2xl border border-jup-border/50">
                <div className="flex justify-between text-sm font-bold mb-3">
                  ...
                  <span className="text-jup-text-dim">${mode === 'Mint' ? calculatedOutput : (parseFloat(calculatedOutput) * assetPrice || 0).toFixed(2)}</span>
                </div>
                ...
                <div className="mt-4 text-xs font-bold">
                  <span className="text-jup-text-dim">Balance: {mode === 'Mint' ? '0' : selectedAsset.balance}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 bg-[#3d2408]/30 border border-[#f59e0b]/20 p-4 rounded-2xl flex gap-3">
              ...
              <div className="text-sm">
                <p className="font-bold text-[#f59e0b]">Elevated Minting Fees</p>
                <p className="text-[#f59e0b]/80">Fees have been adjusted due to market conditions. <span className="underline cursor-pointer">Learn more</span></p>
              </div>
            </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="jup-card p-8 bg-[#13151a]/50 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-2 gap-y-10 gap-x-8">
              <div>
                <p className="text-jup-text-dim text-xs font-bold uppercase tracking-widest mb-2">TVL</p>
                <p className="text-2xl font-mono font-bold">$2,450,000</p>
              </div>
              <div>
                <p className="text-jup-text-dim text-xs font-bold uppercase tracking-widest mb-2">SUPPLY</p>
                <div className="flex items-baseline gap-2">
                  <p className="text-2xl font-mono font-bold">1,310,000</p>
                  <span className="text-sm text-jup-text-dim font-bold">USDpi</span>
                </div>
              </div>
              <div>
                <p className="text-jup-text-dim text-xs font-bold uppercase tracking-widest mb-2">CR</p>
                <div className="flex items-center gap-3">
                  <p className="text-2xl font-mono font-bold">187%</p>
                  <span className="bg-emerald-500/10 text-emerald-400 text-[10px] px-2 py-0.5 rounded font-bold uppercase">Healthy</span>
                </div>
              </div>
              <div>
                <p className="text-jup-text-dim text-xs font-bold uppercase tracking-widest mb-2">SUSDPI APY</p>
                <p className="text-2xl font-mono font-bold text-jup-primary">21.4%</p>
              </div>
              
              <div className="col-span-2 h-px bg-jup-border my-2"></div>
              
              <div>
                <p className="text-jup-text-dim text-xs font-bold uppercase tracking-widest mb-2">Staked</p>
                <p className="text-xl font-mono font-bold">$850,000</p>
              </div>
              <div>
                <p className="text-jup-text-dim text-xs font-bold uppercase tracking-widest mb-2">Agents Locked</p>
                <div className="flex items-baseline gap-2">
              ...
              <div>
                <p className="text-jup-text-dim text-xs font-bold uppercase tracking-widest mb-2">Agents</p>
          ...
          <div className="jup-card p-6 bg-gradient-to-br from-jup-primary/5 to-transparent border-jup-primary/20">
            ...
            <p className="text-sm text-jup-text-dim leading-relaxed">
              Pillars Protocol is secured by multiple independent audits and a robust stability pool mechanism. Your assets are protected by over-collateralization.
            </p>
          </div>
        </div>
      </div>
...
const MOCK_POSITIONS = [
  { id: 1, side: 'Long', asset: 'xPI', size: '$2,500', leverage: '5.0x', entryPrice: '$3,842.15', pnl: '+$124.50', pnlPercent: '+4.98%', isPositive: true },
];

const MOCK_HISTORY = [
  { id: 1, side: 'Short', asset: 'xPI', size: '$1,200', leverage: '3.0x', price: '$3,862.10', pnl: '-$45.20', pnlPercent: '-3.76%', isPositive: false, date: '2024-03-15 14:20' },
  { id: 2, side: 'Long', asset: 'xPI', size: '$5,000', leverage: '10.0x', price: '$3,815.80', pnl: '+$840.00', pnlPercent: '+16.8%', isPositive: true, date: '2024-03-14 09:15' },
];
...
const LeverageView = ({ 
  ...
  const [selectedAsset, setSelectedAsset] = useState(ASSETS[2]); // Default to xETH
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <div className={cn("w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black text-black", side === 'long' ? "bg-jup-primary" : "bg-rose-500")}>
                {side === 'long' ? 'X' : 'S'}
              </div>
              <span className="text-xl font-bold">{side === 'long' ? 'xPI' : 'sPI'}</span>
            </div>
            <div className="flex items-baseline gap-2">
          ...
          <div className="flex gap-2">
            {['1H', '4H', '1D', '1W'].map(t => (
              <button key={t} className="px-3 py-1 rounded-lg bg-jup-secondary text-xs font-bold hover:text-jup-primary transition-colors">
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="h-[400px] w-full bg-jup-secondary/20 rounded-2xl p-4 border border-jup-border">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={CHART_DATA}>
              ...
              <Tooltip 
                contentStyle={{ backgroundColor: '#13151a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }}
          ...
          <div className="flex items-center gap-6 mb-4 border-b border-jup-border">
            <button 
              ...
              className={cn(
                ...
                activeSubTab === 'positions' ? "text-jup-primary" : "text-jup-text-dim hover:text-white"
              )}
            >
              Positions ({positions.length})
              {activeSubTab === 'positions' && <motion.div layoutId="subtab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-jup-primary" />}
            </button>
            <button 
              onClick={() => setActiveSubTab('history')}
              className={cn(
                "pb-2 text-sm font-bold transition-colors relative",
                activeSubTab === 'history' ? "text-jup-primary" : "text-jup-text-dim hover:text-white"
              )}
            >
              History ({historyCount})
              {activeSubTab === 'history' && <motion.div layoutId="subtab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-jup-primary" />}
            </button>
          </div>
            ...
            {activeSubTab === 'positions' ? (
              <table className="w-full text-left border-separate border-spacing-y-2">
                ...
                <tbody className="text-sm">
                  {positions.map(pos => (
                    <tr key={pos.id} className="bg-white/[0.02] border border-jup-border/50 group hover:bg-white/5 transition-colors">
                      <td className="py-4 text-center rounded-l-xl border-y border-l border-jup-border/30">
                        <div className="flex items-center justify-center gap-2">
                          <span className={cn("px-1.5 py-0.5 rounded text-[10px] font-black uppercase", pos.side === 'Long' ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400")}>
                            {pos.side}
                          </span>
                          <span className="font-bold text-white">{pos.asset}</span>
                        </div>
                      </td>
                      <td className="py-4 font-mono font-bold text-white text-center border-y border-jup-border/30">{pos.size}</td>
                      <td className="py-4 font-mono font-bold text-white text-center border-y border-jup-border/30">{pos.leverage}</td>
                      <td className="py-4 font-mono font-bold text-white text-center border-y border-jup-border/30">{pos.entryPrice}</td>
                      <td className={cn("py-4 text-center font-mono font-bold rounded-r-xl border-y border-r border-jup-border/30", pos.isPositive ? "text-emerald-400" : "text-rose-400")}>
              ...
              <table className="w-full text-left border-separate border-spacing-y-2">
                ...
                <tbody className="text-sm">
                  {MOCK_HISTORY.map(item => (
                    <tr key={item.id} className="bg-white/[0.02] border border-jup-border/50 group hover:bg-white/5 transition-colors">
                      <td className="py-4 text-center rounded-l-xl border-y border-l border-jup-border/30">
                        <div className="flex items-center justify-center gap-2">
                          <span className={cn("px-1.5 py-0.5 rounded text-[10px] font-black uppercase", item.side === 'Long' ? "bg-emerald-500/10 text-emerald-400" : "bg-rose-500/10 text-rose-400")}>
                            {item.side}
                          </span>
                          <span className="font-bold text-white">{item.asset}</span>
                        </div>
                      </td>
                      <td className="py-4 font-mono font-bold text-white text-center border-y border-jup-border/30">{item.size}</td>
                      <td className="py-4 font-mono font-bold text-white text-center border-y border-jup-border/30">{item.leverage}</td>
                      <td className="py-4 font-mono font-bold text-white text-center border-y border-jup-border/30">{item.price}</td>
                      <td className="py-4 text-xs text-jup-text-dim font-bold text-center border-y border-jup-border/30">{item.date}</td>
                      <td className={cn("py-4 text-center font-mono font-bold rounded-r-xl border-y border-r border-jup-border/30", item.isPositive ? "text-emerald-400" : "text-rose-400")}>
        ...
        <div className="flex gap-1 bg-jup-secondary/50 p-1 rounded-xl mb-6">
          <button 
            ...
            className={cn(
              ...
              side === 'long' ? "bg-jup-primary text-black" : "text-jup-text-dim hover:text-white"
            )}
          >
            Long
          </button>
          <button 
            ...
            className={cn(
              "flex-1 py-3 rounded-lg text-lg font-black transition-all",
              side === 'short' ? "bg-rose-500 text-white" : "text-jup-text-dim hover:text-white"
            )}
          >
            Short
          </button>
        ...
        <div className="space-y-6">
          <div className="space-y-2">
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="text-jup-text-dim font-bold font-mono">${(parseFloat(payAmount) * parseFloat(selectedAsset.price.replace('$', '').replace(',', '')) || 0).toFixed(2)}</span>
            </div>
            <div className="relative">
              <input 
                type="number" 
              ...
              >
                <div className={cn("w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold", selectedAsset.color)}>
                  {selectedAsset.icon}
                </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <span className="text-jup-text-dim font-bold text-sm">Leverage</span>
              <span className="text-jup-primary font-mono font-bold text-lg">1.1x</span>
            </div>
          </div>
          ...
          {!isConnected ? (
            ...
            <button 
              ...
              className={cn(
                ...
                side === 'long' ? "bg-jup-primary text-black hover:bg-jup-primary-hover shadow-jup-primary/20" : "bg-rose-500 text-white hover:bg-rose-400 shadow-rose-500/20"
              )}
            >
    ...
    <div className="max-w-5xl mx-auto py-12 px-4">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-jup-text-dim">Provide liquidity or stake assets to earn protocol rewards.</p>
        </div>
        <div className="flex items-center gap-6">
          <div className="text-right">
            <span className="text-xs text-jup-text-dim uppercase font-bold tracking-widest mr-2">Total Earned</span>
            <span className="text-xl font-bold text-white">$0.00</span>
          </div>
          <button className="jup-button-primary py-2 px-6 text-sm hover:brightness-110 active:scale-95 transition-all">Claim All</button>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-4">
        {EARN_POOLS.map((pool) => (
          <div key={pool.id} className="jup-card p-6 flex flex-col md:flex-row items-center justify-between gap-6 hover:border-jup-primary/30 hover:bg-white/[0.02] transition-all cursor-pointer group">
            <div className="flex items-center gap-4 w-full md:w-2/5">
              ...
              <div>
                ...
                <p className="text-sm text-jup-text-dim">{pool.pair} by {pool.protocol}</p>
              </div>
            </div>

            <div className="flex items-center justify-between md:justify-around w-full md:flex-1">
              <div className="text-center">
                <p className="text-xs text-jup-text-dim uppercase font-bold tracking-widest mb-1">APY</p>
                <p className="text-xl font-bold text-emerald-400">{pool.apy}</p>
              </div>
              <div className="text-center">
                <p className="text-xs text-jup-text-dim uppercase font-bold tracking-widest mb-1">TVL</p>
                <p className="text-xl font-bold text-white">{pool.tvl}</p>
              </div>
              <div className="text-center">
                <p className="text-xs text-jup-text-dim uppercase font-bold tracking-widest mb-1">Voted</p>
                <p className="text-xl font-bold text-jup-primary">{pool.voted}</p>
              </div>
              <div className="text-center">
                <p className="text-xs text-jup-text-dim uppercase font-bold tracking-widest mb-1">My Deposit</p>
...
const StakeView = ({ 
  ...
  const [sUsdPiBalance, setSUsdPiBalance] = useState(450.20);
  const [showModeDropdown, setShowModeDropdown] = useState(false);
  const [showSlippageDropdown, setShowSlippageDropdown] = useState(false);
  const [slippage, setSlippage] = useState('0.2%');
        ...
        <div className="lg:col-span-2 jup-card p-8 flex flex-col">
          <div className="mb-6">
            ...
            <p className="text-sm text-jup-text-dim mt-1">
              sUSDpi represents USDpi deposited in the stability pool, accruing returns from protocol-held assets
            </p>
          </div>
          ...
          <div className="flex items-center justify-between mb-8 border-b border-jup-border/50">
            <div className="flex items-center gap-8">
              <button 
                ...
                className={cn(
                  ...
                  activeMetric === 'price' ? "text-jup-primary" : "text-jup-text-dim hover:text-white"
                )}
              >
                Price
                <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded-full font-black">$1.284</span>
                {activeMetric === 'price' && <motion.div layoutId="metric-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-jup-primary" />}
              </button>
              <button 
                onClick={() => setActiveMetric('apy')}
                className={cn(
                  "pb-3 text-lg font-bold transition-all relative flex items-center gap-2",
                  activeMetric === 'apy' ? "text-jup-primary" : "text-jup-text-dim hover:text-white"
                )}
              >
                APY
                <span className="text-xs bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded-full font-black">16.19%</span>
                {activeMetric === 'apy' && <motion.div layoutId="metric-tab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-jup-primary" />}
              </button>
            </div>
            ...
            <div className="flex items-center gap-6 pb-3">
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-jup-text-dim font-bold text-xs uppercase tracking-wider">TVL</span>
                <span className="font-mono font-bold text-white text-sm">$9,690,892</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-jup-text-dim font-bold text-xs uppercase tracking-wider">CR</span>
                <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 rounded-lg font-black text-xs">148.88%</span>
              </div>
            </div>
          </div>
            ...
            {['24 hours', '7 days', '30 days'].map(t => (
              <button 
                ...
                className={cn(
                  ...
                  timeframe === t ? "bg-jup-primary/20 text-jup-primary" : "bg-jup-secondary/30 text-jup-text-dim hover:text-white"
                )}
              >
                {t}
              </button>
          ...
          <div className="flex-1 min-h-[400px] relative">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={STAKE_CHART_DATA}>
                ...
                <XAxis dataKey="time" stroke="#94a3b8" fontSize={12} tickLine={false} axisLine={false} />
                <YAxis 
                  domain={activeMetric === 'price' ? ['dataMin - 0.05', 'dataMax + 0.05'] : ['dataMin - 1', 'dataMax + 1']} 
                  stroke="#94a3b8" 
                  fontSize={12} 
                  tickLine={false} 
                  axisLine={false} 
                  orientation="right"
                  tickFormatter={(val) => activeMetric === 'price' ? `$${val.toFixed(3)}` : `${val}%`}
                />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#13151a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px' }}
                  itemStyle={{ color: '#c7f284' }}
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                ...
                >
                  {mode} <ChevronDown size={16} className={cn("transition-transform", showModeDropdown && "rotate-180")} />
                </button>
          ...
          <div className="space-y-6">
            <AnimatePresence mode="wait">
              ...
              >
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-jup-text-dim font-bold">You supply</span>
                    <span className="text-jup-text-dim font-bold font-mono">${(parseFloat(amount) * (mode === 'stake' ? 1 : sUsdPiPrice) || 0).toFixed(2)}</span>
                  </div>
                    <div className="bg-jup-secondary/40 p-5 rounded-2xl border border-jup-border/50">
                      ...
                      <div className="flex justify-between items-center text-xs">
                        <div className="flex items-center gap-1.5 text-jup-text-dim font-bold">
                          <Wallet size={12} className="text-jup-primary" />
                          <span>{mode === 'stake' ? usdPiBalance.toLocaleString() : sUsdPiBalance.toLocaleString()}</span>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={handleHalf} className="text-jup-text-dim hover:text-jup-primary transition-colors font-bold">HALF</button>
                          <button onClick={handleMax} className="text-jup-text-dim hover:text-jup-primary transition-colors font-bold">MAX</button>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-jup-text-dim font-bold font-mono">${(parseFloat(amount) * (mode === 'stake' ? sUsdPiPrice : 1) || 0).toFixed(2)}</span>
                  </div>
                  <div className="bg-jup-secondary/40 p-5 rounded-2xl border border-jup-border/50">
                    <div className="flex justify-between items-center mb-4">
                      <div className="flex items-center gap-2 bg-jup-secondary px-3 py-1.5 rounded-xl border border-jup-border/50">
                      ...
                      <input 
                        ...
                        placeholder="0" 
                        readOnly
                        value={amount ? (mode === 'stake' ? parseFloat(amount) / 1.284 : parseFloat(amount) * 1.284).toFixed(2) : ''}
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <div className="flex items-center gap-1.5 text-jup-text-dim font-bold">
                        <Wallet size={12} className="text-jup-primary" />
                        <span>{mode === 'stake' ? sUsdPiBalance.toLocaleString() : usdPiBalance.toLocaleString()}</span>
                      </div>
                    </div>
                  </div>
                </div>
  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-6">
      <div className="mb-12">
        ...
        <p className="text-jup-text-dim text-lg">Real-time overview of the Pillars ecosystem.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {stats.map((stat, i) => (
          <div key={i} className="jup-card p-6">
            <p className="text-xs text-jup-text-dim uppercase font-bold tracking-widest mb-4 text-center">{stat.label}</p>
            <div className="flex flex-col items-center">
              <p className="text-[40px] leading-tight font-black">{stat.value}</p>
              <p className="text-sm text-emerald-400 font-bold mt-1">{stat.change}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {sections.map((section, i) => (
          <div key={i} className="jup-card p-8">
            <h2 className="text-2xl font-black mb-8 border-b border-jup-border/50 pb-4 text-center">{section.title}</h2>
            <div className="space-y-6">
              {section.metrics.map((metric, j) => (
                <div key={j} className="flex justify-between items-center">
                  <span className="text-jup-text-dim font-bold">{metric.label}</span>
      ...
      <footer className="py-12 px-6 border-t border-jup-border bg-jup-card/20">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2 opacity-50">
            <USDpiIcon size={32} />
            <span className="font-bold uppercase tracking-widest text-lg">PILLARS</span>
          </div>
          
          <div className="flex gap-10 text-sm text-jup-text-dim">
            <a href="#" className="hover:text-white transition-colors" title="Documentation">
              <FileText size={26} />
            </a>
            ...
            <a href="#" className="hover:text-white transition-colors" title="Terms of Service">
              <Shield size={26} />
            </a>
          </div>

          <div className="flex items-center gap-2 text-xs text-jup-text-dim">
          </div>
        </div>
      </footer>
    </div>
